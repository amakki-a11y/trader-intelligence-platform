...
function AccountDetail({ account, version, onBack }: AccountDetailProps) {
  ...
  const [acctInfoExpanded, setAcctInfoExpanded] = useState(true);
    ...
    (async () => {
      try {
        ...
        setOpenTrades(data.map((p: PositionResponse) => ({
          ticket: p.positionId, time: p.time, symbol: p.symbol ?? "", action: p.action ?? "BUY",
          volume: p.volume ?? 0, openPrice: p.priceOpen ?? 0, currentPrice: p.priceCurrent ?? 0,
          profit: p.profit ?? 0, swap: p.swap ?? 0, sl: p.sl ?? 0, tp: p.tp ?? 0,
        })));
      } catch (err: unknown) {
        if (err instanceof DOMException && err.name === "AbortError") return;
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      wsRef.current = ws;
      ws.onopen = () => { attempt = 0; ws.send(JSON.stringify({ subscribe: ["positions", "deals"] })); };
      ws.onmessage = (ev) => {
        try {
          ...
          if (msg.type === "positions") {
            ...
            setOpenTrades(prev => {
              ...
              if (idx >= 0) {
                const updated = [...prev];
                updated[idx] = { ...updated[idx]!, volume: p.volume, currentPrice: p.currentPrice, profit: p.unrealizedPnl, swap: p.swap };
                return updated;
              }
              return [...prev, { ticket: p.positionId, time: new Date().toISOString(), symbol: p.symbol, action: p.direction === 0 ? "BUY" : "SELL", volume: p.volume, openPrice: p.openPrice, currentPrice: p.currentPrice, profit: p.unrealizedPnl, swap: p.swap, sl: 0, tp: 0 }];
            });
          }
          if (msg.type === "deals") {
      ...
      ws.onclose = () => { wsRef.current = null; const delay = Math.min(1000 * Math.pow(2, attempt), 15000); attempt++; reconnectTimer = setTimeout(connect, delay); };
      ws.onerror = () => ws.close();
    };
    connect();
    return () => { clearTimeout(reconnectTimer); ws?.close(); wsRef.current = null; };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [account.login, refetchPositions]);

  const moneyOps = useMemo((): MoneyOp[] => {
    return deals
      .filter(d => ["BALANCE", "CREDIT", "BONUS"].includes(d.action))
      .map((d) => ({
        id: d.ticket, time: d.time,
        type: d.action === "BALANCE" ? (d.profit >= 0 ? "Deposit" : "Withdrawal") : d.action === "CREDIT" ? "Credit" : "Bonus",
        amount: Math.abs(d.profit), method: "", status: "Completed",
      }));
  }, [deals]);
  ...
  const localTh: CSSProperties = { ...sharedTh, padding: "6px 6px" };
  const localTd: CSSProperties = { ...sharedTd, padding: "5px 6px" };
  ...
  const pagedDeals = sortedDeals.slice(page * pageSize, (page + 1) * pageSize);
  const pageStart = page * pageSize + 1;
  const pageEnd = Math.min((page + 1) * pageSize, sortedDeals.length);
    ...
    <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <div style={{ overflow: "auto", flex: 1, padding: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 16, fontSize: 12 }}>
          <span onClick={onBack} style={{ color: C.teal, cursor: "pointer" }}>Accounts</span>
          <span style={{ color: C.t3 }}>&gt;</span>
          <span style={{ color: C.t1, fontFamily: "'JetBrains Mono',monospace", fontWeight: 600 }}>{account.login}</span>
          <span style={{ color: C.t3 }}>&mdash;</span>
          <span style={{ color: C.t2 }}>{account.name}</span>
        </div>
        {/* Header */}
        ...
        {/* Open Trades */}
        {tab === "open" && (<div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
          <div style={{ flex: 1, overflow: "auto", minHeight: 0 }}>
            {openTrades.length === 0 ? (<div style={{ padding: 30, textAlign: "center", color: C.t3, fontSize: 12 }}>No open positions</div>) : (
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead><tr>{["Ticket","Time","Symbol","Action","Vol","Open Price","Current","Profit","Swap","S/L","T/P"].map(h => {
                  const isNum = ["Vol","Open Price","Current","Profit","Swap","S/L","T/P"].includes(h);
                  return <th key={h} style={{ ...localTh, textAlign: isNum ? "right" : "left" }}>{h}</th>;
                })}</tr></thead>
                <tbody>
                  {openTrades.map((t, idx) => (<tr key={t.ticket} style={{ background: idx % 2 === 1 ? "rgba(255,255,255,0.015)" : "transparent" }}>
                    <td style={localTd}>{t.ticket}</td>
                    <td style={localTd}>{new Date(t.time).toLocaleString("en-GB", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}</td>
                    <td style={localTd}>{t.symbol}</td>
                    <td style={{ ...localTd, color: t.action === "BUY" ? C.blue : C.red }}>{t.action}</td>
                    <td style={{ ...localTd, textAlign: "right" }}>{t.volume}</td>
                    <td style={{ ...localTd, textAlign: "right" }}>{t.openPrice}</td>
                    <td style={{ ...localTd, textAlign: "right" }}>{t.currentPrice}</td>
                    <td style={{ ...localTd, color: t.profit >= 0 ? C.green : C.red, fontWeight: 600, textAlign: "right" }}>{t.profit >= 0 ? "+" : ""}{t.profit}</td>
                    <td style={{ ...localTd, textAlign: "right" }}>{t.swap}</td>
                    <td style={{ ...localTd, color: C.red, textAlign: "right" }}>{t.sl}</td>
                    <td style={{ ...localTd, color: C.green, textAlign: "right" }}>{t.tp}</td>
                  </tr>))}
                  <tr><td colSpan={7} style={{ ...localTd, textAlign: "right", fontSize: 10, color: C.t3 }}>TOTAL P&L:</td>
                    <td style={{ ...localTd, fontWeight: 700, color: totalOpenPnl >= 0 ? C.green : C.red, textAlign: "right" }}>{totalOpenPnl >= 0 ? "+" : ""}{totalOpenPnl.toFixed(2)}</td><td colSpan={3} style={localTd} /></tr>
                </tbody>
              </table>)}
          </div>
        </div>)}
        ...
        {tab === "history" && (() => {
          ...
          const numCols = ["volume", "price", "profit", "commission", "expertId"];
          return (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead><tr>{histCols.map(h => (
                  <th key={h.key} onClick={() => toggleDealSort(h.key)}
                    style={{ ...localTh, cursor: "pointer", textAlign: numCols.includes(h.key) ? "right" : "left" }}>
                ...
                <tbody>
                  {pagedDeals.map((d, idx) => {
                    ...
                    return (
                    <tr key={d.ticket} style={{ background: idx % 2 === 1 ? "rgba(255,255,255,0.015)" : "transparent" }}>
                      <td style={localTd}>{d.ticket}</td>
                      <td style={localTd}>{new Date(d.time).toLocaleString("en-GB", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}</td>
                      <td style={{ ...localTd, color: entryColor, fontWeight: 600, fontSize: 10 }}>{d.entry || "\u2014"}</td>
                      <td style={localTd}>{d.symbol}</td>
                      <td style={{ ...localTd, color: d.action === "BUY" ? C.blue : d.action === "SELL" ? C.red : C.t2 }}>{d.action}</td>
                      <td style={{ ...localTd, textAlign: "right" }}>{d.volume}</td>
                      <td style={{ ...localTd, textAlign: "right" }}>{d.price}</td>
                      <td style={{ ...localTd, color: d.profit >= 0 ? C.green : C.red, textAlign: "right" }}>{d.profit >= 0 ? "+" : ""}{d.profit}</td>
                      <td style={{ ...localTd, textAlign: "right" }}>${d.commission}</td>
                      <td style={localTd}>{d.reason}</td>
                      <td style={{ ...localTd, textAlign: "right" }}>{d.expertId || "\u2014"}</td>
                    </tr>
                    );
                  })}
                </tbody>
            ...
            {sortedDeals.length > 0 && (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderTop: `1px solid ${C.border}` }}>
                <span style={{ fontSize: 10, color: C.t3, fontFamily: "'JetBrains Mono',monospace" }}>
                  Showing {pageStart}-{pageEnd} of {sortedDeals.length} deals
                </span>
                <div style={{ display: "flex", gap: 6 }}>
                  <button disabled={page === 0} onClick={() => setPage(p => p - 1)} style={{ padding: "3px 10px", borderRadius: 4, border: `1px solid ${C.border}`, background: page === 0 ? "transparent" : C.bg3, color: page === 0 ? C.t3 : C.t1, fontSize: 10, cursor: page === 0 ? "default" : "pointer", fontFamily: "'JetBrains Mono',monospace" }}>Prev</button>
                  <span style={{ fontSize: 10, color: C.t2, fontFamily: "'JetBrains Mono',monospace", padding: "3px 6px" }}>Page {page + 1} of {totalPages}</span>
                  <button disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} style={{ padding: "3px 10px", borderRadius: 4, border: `1px solid ${C.border}`, background: page >= totalPages - 1 ? "transparent" : C.bg3, color: page >= totalPages - 1 ? C.t3 : C.t1, fontSize: 10, cursor: page >= totalPages - 1 ? "default" : "pointer", fontFamily: "'JetBrains Mono',monospace" }}>Next</button>
                </div>
              </div>
            )}
          </div>
        ...
        {tab === "deposits" && (
          <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
            <div style={{ flex: 1, overflow: "auto", minHeight: 0 }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead><tr>{["ID","Date","Type","Amount","Method","Status"].map(h => {
                  const isNum = h === "Amount";
                  return <th key={h} style={{ ...localTh, textAlign: isNum ? "right" : "left" }}>{h}</th>;
                })}</tr></thead>
                <tbody>
                  {moneyOps.map((op, idx) => {
                    ...
                    return (
                      <tr key={op.id} style={{ background: idx % 2 === 1 ? "rgba(255,255,255,0.015)" : "transparent" }}>
                        <td style={localTd}>{op.id}</td>
                        <td style={localTd}>{new Date(op.time).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</td>
                        <td style={{ ...localTd, color: typeColor, fontWeight: 600 }}>{op.type}</td>
                        <td style={{ ...localTd, color: typeColor, textAlign: "right" }}>{op.type === "Withdrawal" ? "-" : "+"}${op.amount.toLocaleString()}</td>
                        <td style={localTd}>{op.method}</td>
                        <td style={localTd}><span style={{ color: C.green }}>{"\u2713"}</span> {op.status}</td>
                      </tr>
                    );
                  })}
                  <tr>
                    <td colSpan={3} style={{ ...localTd, textAlign: "right", fontSize: 10, color: C.t3 }}>NET DEPOSITS:</td>
                    <td style={{ ...localTd, fontWeight: 700, color: C.teal, textAlign: "right" }}>
                      ${moneyOps.reduce((s, o) => s + (o.type === "Withdrawal" ? -o.amount : o.amount), 0).toLocaleString()}
                    </td>
                    <td colSpan={2} style={localTd} />
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        {/* AI Routing Tab */}
        {tab === "ai" && (<AIRoutingPanel login={account.login} />)}
      </div>
    </div>
  );
}
